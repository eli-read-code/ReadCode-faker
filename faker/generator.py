...
class Generator:
    ...
    def add_provider(self, provider: "BaseProvider") -> None:
        ...
        self.providers.insert(0, provider) # eli:: 插在列表第一个位置
    ...
    def get_formatter(self, formatter: str) -> Callable:
        try:
            ...
            print("eli::first_name=", getattr(self,"first_name"))
            print("eli::city_suffix=", getattr(self,"city_suffix"))
            eli::first_name= <bound method Provider.first_name of <faker.providers.person.zh_CN.Provider object at 0x000002548BD0E670>>
            eli::city_suffix= <bound method Provider.city_suffix of <faker.providers.address.zh_CN.Provider object at 0x000002548BC4D310>>
            eli::first_name= <bound method Provider.first_name of <faker.providers.person.zh_CN.Provider object at 0x000002548BD0E670>>
            eli::city_suffix= <bound method Provider.city_suffix of <faker.providers.address.zh_CN.Provider object at 0x000002548BC4D310>>
            """
    ...
    def parse(self, text: str) -> str:
        ...
        """
        # print("eli::",_re_token.sub(self.__format_token, text))
        # eli::[out]eli:: 军市
        # _re_token = re.compile(r"\{\{\s*(\w+)(:\s*\w+?)?\s*\}\}")

        # print("eli::",text)
        # eli::[out] eli:: {{first_name}}{{city_suffix}}
        # eli::__format_token is Callable,是匹配字串的回调处理函数
        return _re_token.sub(self.__format_token, text)

    def __format_token(self, matches):
        # eli:: 每一个匹配项,都会调用一次这个函数.
        """
        eli::[out]
        print("eli::",matches)
        eli:: <re.Match object; span=(0, 14), match='{{first_name}}'>
        eli:: <re.Match object; span=(14, 29), match='{{city_suffix}}'>
        """
        formatter, argument_group = list(matches.groups())
        """
        eli::[out]
        print("eli::",formatter)
        print("eli::",argument_group)
        eli:: city_name
        eli:: None
        eli:: city_suffix
        eli:: None
